   ...
   this.http
    ...
    .subscribe((data) => {
     ...
     for (let i = 0; i < this.childOptionsList.length; i++) {
      ...
      $("#" + element_id).append($("<option></option>").attr("value", -1).attr("selected", true).text("Please Select"));
      ...
      $.each(child_element_options, function (key, value) {
       ...
       // );
       $("#" + element_id).append($("<option></option>").attr("value", value.opt_id).attr("data-val", value.opt_selected_value).text(value.opt_value));
      });
     }
    });
  }
